#!/usr/bin/env python3
"""Extract pet row strips without resizing each pose independently.

The hatch-pet extractor fits every pose to the output cell after cropping its
alpha bounding box. That is useful for isolating sprites, but it makes crouched
and airborne poses change apparent size. This extractor groups each row into
frames, chooses one scale for the whole row, and preserves the row's original
vertical offsets inside the 192x208 Codex pet cell.
"""
...
class Component:
    """One connected alpha component in a decoded row strip."""
...
@dataclass(frozen=True)
class FrameSource:
    """A sprite frame plus its original row-space bounding box."""
...
def source_from_components(
    ...
    return FrameSource(output, (min_x, min_y, max_x, max_y))


def component_frame_sources(strip: Image.Image, frame_count: int, padding: int) -> list[FrameSource]:
    components = connected_components(strip)
    if not components:
        raise SystemExit("no sprite components found")
    ...
    if len(seeds) < frame_count:
        raise SystemExit(f"could not find {frame_count} sprite components")
    ...
    groups: list[list[Component]] = [[seed] for seed in seeds]
    ...
    return [source_from_components(strip, group, padding) for group in groups]


def slot_frame_sources(strip: Image.Image, frame_count: int) -> list[FrameSource]:
    ...
    for index in range(frame_count):
        ...
        bbox = crop.getbbox()
        if bbox is None:
            sources.append(FrameSource(Image.new("RGBA", (1, 1), (0, 0, 0, 0)), (left, 0, left, 0)))
            continue
        absolute_bbox = (left + bbox[0], bbox[1], left + bbox[2], bbox[3])
        sources.append(FrameSource(crop.crop(bbox), absolute_bbox))
    return sources
...
def scale_for_row(
    ...
    *,
    ...
    nonempty = [source for source in sources if source.image.getbbox()]
    if not nonempty:
        return 1.0

    max_sprite_width = max(source.image.width for source in nonempty)
    max_sprite_height = max(source.image.height for source in nonempty)
    row_min_y = min(source.bbox[1] for source in nonempty)
    row_max_y = max(source.bbox[3] for source in nonempty)
    row_span_height = max(1, row_max_y - row_min_y)
    return min(
        target_width / max_sprite_width,
        target_height / max_sprite_height,
        (CELL_HEIGHT - 2 * cell_margin) / row_span_height,
        1.0,
    )
...
def fit_row_sources(
    ...
    *,
    ...
    nonempty = [source for source in sources if source.image.getbbox()]
    if not nonempty:
        return [Image.new("RGBA", (CELL_WIDTH, CELL_HEIGHT), (0, 0, 0, 0)) for _ in sources]
    ...
    row_min_y = min(source.bbox[1] for source in nonempty)
    row_max_y = max(source.bbox[3] for source in nonempty)
    row_scaled_height = (row_max_y - row_min_y) * scale
    row_top = (CELL_HEIGHT - row_scaled_height) / 2

    frames: list[Image.Image] = []
    for source in sources:
        target = Image.new("RGBA", (CELL_WIDTH, CELL_HEIGHT), (0, 0, 0, 0))
        if not source.image.getbbox():
            frames.append(target)
            continue

        resized_width = max(1, round(source.image.width * scale))
        resized_height = max(1, round(source.image.height * scale))
        resized = source.image.resize((resized_width, resized_height), Image.Resampling.LANCZOS)
        left = (CELL_WIDTH - resized.width) // 2
        top = round(row_top + (source.bbox[1] - row_min_y) * scale)
...
def extract_state(
    ...
    if method == "slots":
        sources = slot_frame_sources(strip, frame_count)
        used_method = "slots"
    else:
        sources = component_frame_sources(strip, frame_count, padding=4)
        used_method = "components"

    frames = fit_row_sources(
        sources,
        target_height=target_height,
        target_width=target_width,
        cell_margin=cell_margin,
...
def main() -> None:
    ...
    parser.add_argument("--method", choices=("components", "slots"), default="components")
    parser.add_argument("--target-height", type=int, default=170)
    parser.add_argument("--target-width", type=int, default=170)
    parser.add_argument("--cell-margin", type=int, default=5)
